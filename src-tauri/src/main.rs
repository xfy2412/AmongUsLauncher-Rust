...
// 检查服务器状态
async fn check_server_connectivity(host: &str, port: u16, timeout_ms: u64) -> ServerStatusResult {
    ...
    if socket_addrs.is_empty() {
        return ServerStatusResult {
            online: false,
            response_time: None,
            error: Some("无法解析服务器地址".to_string())
        };
    }
...
async fn check_server_status(host: String, port: u16) -> Result<ServerStatusResult, String> {
    let result = check_server_connectivity(&host, port, 5000).await;
    Ok(result)
}
...
fn remove_server_by_index(
    ...
    if let Some(servers) = existing_regions[region_index].get_mut("Servers") {
        if let Some(servers_array) = servers.as_array_mut() {
            // 检查服务器索引是否有效
            if server_index >= servers_array.len() {
                return Err(format!("无效的服务器索引: {}", server_index));
            }
            
            // 删除指定索引的服务器
            servers_array.remove(server_index);
            ...
            return Err("区域中没有Servers字段或字段不是数组".to_string());
        }
    } else {
        return Err("区域中没有Servers字段".to_string());
    }
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![get_servers, get_now_server, add_server, remove_server, remove_server_by_index, check_server_status])
